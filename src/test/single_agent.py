...
for DATA in DATASETS:
    ...
    for sample in tqdm(data):
        ...
        resp, _, _ = evaluate(sample['0']['responses'], sample['0']['answer'])
        agent_corr = [x == sample['0']['answer'] for x in resp]
        
        sample_list.append(agent_corr)
        
    sample_list = np.array(sample_list)
    agent_performance = sample_list.mean(0)
    single_agent_mean, single_agent_std = agent_performance.mean(), agent_performance.std()
    
    print(single_agent_mean, single_agent_std)