...
def load_data(args, split=None, easy=False):
    ...
    for i in range(0, num_params * data_size, num_params):
        if easy :
            ...
            answer = np.round(a + b * c - d)
        else :
            a, b, c, d, e, f = x[i:i+6]
            if f == 0 : 
                f = 1
            question = f'What is the result of {a}+{b}*{c}+{d}-{e}÷{f}?'
            answer = np.round(a + b * c + d - e / f)
        X.append(question)
        Y.append(answer)
    
    if easy:
        with open(f"out/data_cache/easy_arithmetics_{data_size}.pkl", "wb") as f:
            pickle.dump([X, Y], f)
    else:
        with open(f"out/data_cache/arithmetics_{data_size}.pkl", "wb") as f:
            pickle.dump([X, Y], f)
