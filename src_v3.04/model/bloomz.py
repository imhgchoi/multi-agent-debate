...
class BloomzWrapper(object):
    ...
    def generate(self, args, query, max_new_tokens=64, return_logits=True, verbose=False):
        ...
        tokenizer = self.align_target_tokenizer if self.align_target_tokenizer is not None else self.tokenizer
        gen_responses = [''] * len(query)
        for score in gen.scores:
            toks = tokenizer.convert_ids_to_tokens(score.argmax(1))
