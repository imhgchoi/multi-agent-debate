...
def get_new_message(args, sample, responses, suffix=None):
    ...
    if len(agents) > 1 : # MULTI-AGENT DEBATE
        ...
        if args.centralized == 'none' : # DECENTRALIZED MAD
            ...
            center_agent_idx = [x.split("__")[1] for x in agents].index(args.centralized)
            for i, agent in enumerate(agents):
...
def run_debate(args, query, agents, agent_names, evaluator, answer=-1.234, suffix="", verbose=True):
    ...
    if args.data in ['arithmetics','hellaswag','pro_medicine','gsm8k']:
